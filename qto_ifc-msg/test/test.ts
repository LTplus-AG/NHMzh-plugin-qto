...
function createMockHttpServer(port: number = 8000): http.Server {
	const server = http.createServer((req, res) => {
		...
		req.on("end", () => {
			...
			console.log(`Mock server received request: ${JSON.stringify(requestData, null, 2)}`);

			// Send a success response
			res.writeHead(200, { "Content-Type": "application/json" });
			res.end(JSON.stringify({ status: "success", message: "Request received" }));
		});
	});

	server.listen(port, "0.0.0.0", () => {
		console.log(`Mock HTTP server started on port ${port}, listening on all interfaces`);
	});
...
function initialize(): void {
	...
	console.log(`Using backend URL: ${backendUrl}, starting mock server on port: ${backendPort}`);
...
async function uploadIfcFilesToMinio(): Promise<void> {
	...
	const content = fs.readFileSync(path.join(__dirname, "..", "test", "assets", "test.ifc"));

	for (const msg of messages) {
		await minioClient.putObject(ifcBucket, msg.fileNameUUID, content, content.length, {
		...
		console.log(`Uploaded file to MinIO: ${msg.fileNameUUID}, size: ${content.length} with metadata`);
	}
}
...
async function main(): Promise<void> {
	...
	console.log("Adding IFC files to MinIO directly and sending Kafka messages...");
	try {
		...
		console.error(`Error adding IFC files to MinIO and sending Kafka messages: ${err}`);
		process.exit(1);
	}
